...
host_interface = mule_zabbix.get_interface(z,jta_server_hostID)
log_interfaceID = mule_zabbix.get_interface(z,jta_server_hostID)
print "list of interfaces \n"
print log_interfaceID
#####################################################
#  Check for the JTA's and add the missing ones     #
#####################################################
deployed_jta_applicaiton_list = mule_zabbix.get_jta_applicaiton_list(z,jta_server_hostID,"jta")
...
if not deployed_jta_list['deployed_apps']:
    ...
    for item in deployed_jta_list['deployed_apps']:
        if item in deployed_jta_applicaiton_list:
            ...
            #mule_zabbix.get_interface(z,jta_server_hostID)
            mule_zabbix.add_jta_jmx_items(z,item,jta_jmx_interfaceID,new_jta_applicationID,jta_server_hostID)
